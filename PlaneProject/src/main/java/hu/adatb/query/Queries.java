...
public class Queries {
    ...
    public static final String SELECT_FLIGHT = "SELECT JARAT.ID, JARAT.FELSZALLAS_DATUM, p1.NEV as from_airport, p2.NEV as to_airport, r1.NEV as plane_name, JARAT.SZABAD_HELYEK FROM JARAT LEFT JOIN REPULOTER p1 ON JARAT.REPULOTER_ID_FEL = p1.ID LEFT JOIN REPULOTER p2 ON JARAT.REPULOTER_ID_LE = p2.ID LEFT JOIN REPULOGEP r1 ON JARAT.REPULOGEP_ID = r1.ID";
    public static final String SELECT_FLIGHT_BY_ID = "SELECT JARAT.ID, JARAT.FELSZALLAS_DATUM, p1.NEV as from_airport, p2.NEV as to_airport, r1.NEV as plane_name, JARAT.SZABAD_HELYEK FROM JARAT LEFT JOIN REPULOTER p1 ON JARAT.REPULOTER_ID_FEL = p1.ID LEFT JOIN REPULOTER p2 ON JARAT.REPULOTER_ID_LE = p2.ID LEFT JOIN REPULOGEP r1 ON JARAT.REPULOGEP_ID = r1.ID WHERE JARAT.ID = ";
